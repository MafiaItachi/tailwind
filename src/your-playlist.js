...
async function addPlaylist() {
  const playlistLink = document
    .getElementById("playlistLinkInput")
    .value.trim();
...
async function syncPlaylists() {
  ...
  for (const playlistKey in savedPlaylists) {
    ...
    const newSongs = updatedSongs.filter(
      (newSong) =>
        !existingSongs.some((existingSong) => existingSong.id === newSong.id)
    );
...
function revealSongs(playlistKey) {
  ...
  const playlistThumbnail = songs[0]
    ? `https://img.youtube.com/vi/${songs[0].id}/mqdefault.jpg`
    : "";
  ...
  favoriteButton.addEventListener("click", () => {
    if (playlists[cleanedPlaylistKey]) {
      ...
      // Add to favorites
      playlists[cleanedPlaylistKey] = songs.map((song) => ({
        id: song.id,
        title: song.title,
      }));
      localStorage.setItem("playlists", JSON.stringify(playlists));
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var addedPlaylistsElements = document.getElementsByClassName(
    "yourplaylist-thumbnail"
  );
...
function togglePlaylistContainerVisibility() {
  ...
  if (addedPlaylistsContainer) {
    addedPlaylistsContainer.style.display = isPlaylistContainerVisible
      ? "block"
      : "none";
  }
}
...
function toggleFavoriteArtistsContainerVisibility() {
  ...
  if (favoriteArtistsContainer) {
    favoriteArtistsContainer.style.display = isFavoriteArtistsContainerVisible
      ? "block"
      : "none";
  }
}
